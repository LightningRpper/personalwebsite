class DownloadersController < ApplicationController
  ...
  def create
    ...
    respond_to do |format|
      if @downloader.save
        format.html { redirect_to downloader_url(@downloader), notice: "Downloader was successfully created." }
  ...
  def update
    respond_to do |format|
      if @downloader.update(downloader_params)
        format.html { redirect_to downloader_url(@downloader), notice: "Downloader was successfully updated." }
  ...
  private
    ...
    def downloader_params
      params.require(:downloader).permit(:title)
    end
end
